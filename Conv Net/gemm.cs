...
namespace Conv_Net {
    ...
    class test_gemm {
        ...
        public test_gemm() {
            ...
            // Input: 2 samples x 4 rows x 4 columns x 3 channels
            // Filters: 2 num x 2 rows x 2 columns x 3 channels
            // Output size: 3 rows x 3 columns x 2 channels
                ...
                for (int j = 0; j < I.dim_2; j++) {
                    for (int k = 0; k < I.dim_3; k++) {
                        for (int l = 0; l < I.dim_4; l++) {
                            this.I.values[this.I.index(i, j, k, l)] = l * I.dim_2 * I.dim_3 + j * I.dim_3 + k + 1;
                        }
                    }
                }
            }
            ...
            this.Conv = new Convolution_Layer(3, 2, 2, 2, true);
            this.MSE = new Mean_Squared_Loss();

            for (int i=0; i < Conv.F.dim_1; i++) {
                for (int j=0; j < Conv.F.dim_2; j++) {
                    for (int k=0; k < Conv.F.dim_3; k++) {
                        for (int l = 0; l < Conv.F.dim_4; l++) {
                            Conv.F.values[Conv.F.index(i, j, k, l)] = i * Conv.F.dim_2 * Conv.F.dim_3 * Conv.F.dim_4 + l * Conv.F.dim_2 * Conv.F.dim_3 + j * Conv.F.dim_3 + k + 1;
                        }
                    }
                }
            }
            Tensor FF = Conv.F.F_2_col();
            I = I.im_2_col(Conv.F_rows, Conv.F_columns, Conv.F_channels);
